...
class Themify_Builder_Component_Base {
    ...
    private static function get_frame_props($selector = '', $id = 'top') {
		return array(
			array(
				 ...
				 * @ref #7241
				 */
				'selector' => array($selector . '> .tb_row_frame_wrap .tb_row_frame_' . $id, $selector . ' > .fp-tableCell > .tb_row_frame_' . $id),
				'option_js' => true
			),
			array(
				'id' => $id . '-frame_layout',
	...
	protected static function show_frame( $styles, $printed=array()) {
		    ...
		    if(!isset($printed[$side])){
				foreach ( $breakpoints as $bp => $v ) {
					...
					if ( !empty( $settings ) && Themify_Builder_Model::get_frame( $settings, $side ) ) {
						...
						$output .= '<div class="tf_abs tf_hide tb_row_frame tb_row_frame_' . $side . ' ' . $frame_location . '"></div>';
						break;
					}
				}
		   }
		...
		if ( ! empty( $output ) ) {
			echo '<div class="tb_row_frame_wrap tf_abs">' . $output . '</div>';
		}
		return $printed;
	}
    /**
