...
(function ($, Themify, topWindow, document) {
    ...
    ThemifyConstructor = {
		...
		tb_tooltip : {
			render( data, self ) {
				let f = self.create( [
					{ type : 'separator', 'html' : '<div><hr><h4>' + self.label.t + '</h4></div>' },
					{
						type : 'textarea',
						label : self.label.tt,
						id : '_tooltip',
			...
			bindEvents( el ) {
				let self = this;
				let _tooltip = el.querySelector( '#_tooltip' ),
					color_fields = [ el.querySelector( '#_tooltip_bg' ), el.querySelector( '#_tooltip_c' ) ];
				_tooltip.addEventListener( 'focus', () => { self.addOrRemoveTooltip() } );
				_tooltip.addEventListener( 'keyup', () => { self.addOrRemoveTooltip() } );
				_tooltip.addEventListener( 'blur', () => {
					self.addOrRemoveTooltip( false );
				});

				color_fields.forEach( ( color_field ) => {
					color_field.addEventListener( 'themify_builder_color_picker_show', () => {
						self.addOrRemoveTooltip();
						color_field.addEventListener( 'themify_builder_color_picker_hide', () => {
							self.addOrRemoveTooltip( false );
						}, { once : true } );
					});
					color_field.addEventListener( 'themify_builder_color_picker_change', () => { self.addOrRemoveTooltip() } );
				} );

				return el;
			},
			/* creates tooltip preview element */
			addOrRemoveTooltip( show = true ) {
				console.log('addorremo');
				...
				if ( ! Themify.cssLazy['tb_tooltip'] ) {
					Themify.LoadCss( ThemifyBuilderModuleJs.cssUrl + 'tooltip.css' );
					Themify.cssLazy['tb_tooltip'] = true;
				}

				let val = topWindow.document.getElementById( '_tooltip' ).value,
					color = api.Utils.getColor(topWindow.document.getElementById( '_tooltip_c' )),
					bg = api.Utils.getColor(topWindow.document.getElementById( '_tooltip_bg' ));

				if ( val !== '' ) {
					if ( ! tooltip ) {
						tooltip = document.createElement( 'div' );
						tooltip.classList.add( 'tb_tooltip' );
