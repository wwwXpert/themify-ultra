...
class Themify_Custom_Fonts {
	...
	/**
	 * User-agent string to force load .ttf fonts from Google
	 */
	public static $user_agent = 'Mozilla/5.0 (Linux; U; Android 2.2.1; en-ca; LG-P505R Build/FRG83) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1';
		...
		if ( ! empty( $font_files ) ) {
			...
			foreach ( $font_files as $font_name => $variations ) {
				...
				foreach ( $variations as $variation ) {
					...
					$downloaded_font = self::download_external_font( $variation );
					$variations_meta[] = [
	 ...
	 * @return array
	 */
	public static function parse_google_css( string $css ) {
		$fonts = [];
		$font_name = null;
		$font_style = null;
		$font_weight = null;
    ...
    public static function download_external_font( array $font ) {
        $base_dir = trailingslashit( themify_upload_dir()['basedir'] ) . 'themify-fonts/';
        if ( ! is_dir( $base_dir ) ) {
            mkdir( $base_dir );
        }

		$font_file = file_get_contents( $font['url'] );
		$font_file_name = basename( $font['url'] );
		$uploads_dir = $base_dir . sanitize_file_name( $font['font_name'] );

		if ( ! is_dir( $uploads_dir ) ) {
			mkdir( $uploads_dir, 0777, true );
		}

		file_put_contents( $uploads_dir . '/' . $font_file_name, $font_file );
		$url = trailingslashit( themify_upload_dir()['baseurl'] ) . 'themify-fonts/' . sanitize_file_name( $font['font_name'] ) . '/' . $font_file_name;
		return $url;
	}
}
endif;