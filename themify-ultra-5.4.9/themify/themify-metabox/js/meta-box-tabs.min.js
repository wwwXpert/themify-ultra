...
function ilcTabs(a){for(var c in a){var e=a[c].split("_"),b=e[0];jQuery(b+" .ilc-tab:not(:first)").hide();jQuery(b+" .ilc-tab:first").show();jQuery(b+" .ilc-htabs a:first").parents("li").addClass("select");previous[b]=new Reference(jQuery(b+" .ilc-htabs a:first").attr("href").split("#")[1]);current[b]=new Reference(jQuery(b+" .ilc-htabs a").get()[1].href.split("#")[1]);tablist[b]=new Tab(b);void 0!=e[1]&&(tablist[b].intervalid=setInterval("tablist['"+b+"'].next()",e[1]));jQuery(b+" .ilc-htabs a").click(function(d){d=
"#"+d.target.getAttribute("href").split("#")[1];d="#"+jQuery(d).parent().parent().attr("id");current[d].reference=jQuery(this).attr("href").split("#")[1];change(d);clearInterval(tablist[d].intervalid);return!1})}};
