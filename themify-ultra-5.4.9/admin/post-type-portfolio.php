...
if ( ! class_exists( 'Themify_Portfolio' ) ) {
	...
	class Themify_Portfolio {
		 ...
		 * @return string
		 */
		function section_link( $more_link = false, $more_text = '', $post_type = null ) {
			if ( $more_link ) {
				if ( 'true' == $more_link ) {
		...
		function shortcode($atts = array(), $post_type = '' ){
			...
			if( is_singular('portfolio') ){
				...
				if( '' == $post_date ){
					$post_date = themify_get('setting-default_portfolio_index_hide_post_date','yes',true);
				}
				if( '' == $title ){
				...
				// Reverse logic
				if( '' == $post_date ){
					$post_date = 'no' === themify_get('setting-default_portfolio_index_hide_post_date','no',true)?'yes' : 'no';
				}
				if( '' == $post_meta ){
					$post_meta = 'no' === themify_get('setting-default_portfolio_index_post_meta_category','no',true)? 'yes' : 'no';
				}
			}
				...
				$out .= '<div data-lazy="1" class="loops-wrapper shortcode ' .implode(' ',$class) .' tf_clear">';
					...
					if ( 'slider' === $themify->post_layout ) {
						switch ( $speed ) {
							...
							$out .= '<div class="slideshow tf_swiper-container tf_carousel tf_overflow tf_rel" data-lazy="1" data-autoplay="' . $autoplay . '" data-speed="' . $speed . '" data-scroll="' . $scroll . '" data-visible="' . $visible . '"><div class="tf_swiper-wrapper tf_lazy tf_rel tf_w tf_h">';
