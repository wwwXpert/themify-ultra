...
(function ($, window,Themify, document, undefined) {
    ...
    ThemifyBuilderModuleJs = {
        ...
        parallaxScrollingInit: function (el, is_live) {
            ...
            var lax_el = document.querySelector('div[data-lax]'),
                inner_h = $(window.top).height(),
                lax_els = document.querySelectorAll('div[data-lax]');

	    lax_els.forEach(function(item) {
            ...
            if (item.hasAttribute('data-lax-scale')) {
                var entryContent = item.closest('.entry-content');
                if ( entryContent ) {
            ...
            var top = item.getBoundingClientRect().top - document.body.getBoundingClientRect().top;
                if ((top + 130) < inner_h) {
                    if (top < 0) {
                    ...
                    if (item.hasAttribute('data-lax-translate-y')) {
                        var t_y = item.getAttribute('data-lax-translate-y').split(','),
                            ...
                            t_y_end = t_y[1].split(' ');
                        item.setAttribute('data-lax-translate-y', t_y_end[0] + ' ' + t_y_start[1] + ',' + Math.floor(top) + ' ' + t_y_end[1]);
                    }
                    // Horizontal
                    if (item.hasAttribute('data-lax-translate-x')) {
                        var t_x = item.getAttribute('data-lax-translate-x').split(','),
                            ...
                            t_x_end = t_x[1].split(' ');
                        item.setAttribute('data-lax-translate-x', t_x_end[0] + ' ' + t_x_start[1] + ',' + Math.floor(top) + ' ' + t_x_end[1]);
                    }
                    // Opacity
                    if (item.hasAttribute('data-lax-opacity')) {
                        var t_o = item.getAttribute('data-lax-opacity').split(','),
                            ...
                            t_o_end = t_o[1].split(' ');
                        item.setAttribute('data-lax-opacity', t_o_end[0] + ' ' + t_o_start[1] + ',' + Math.floor(top) + ' ' + t_o_end[1]);
                    }
                    // Blur
                    if (item.hasAttribute('data-lax-blur')) {
                        var t_b = item.getAttribute('data-lax-blur').split(','),
                            ...
                            t_b_end = t_b[1].split(' ');
                        item.setAttribute('data-lax-blur', t_b_end[0] + ' ' + t_b_start[1] + ',' + Math.floor(top) + ' ' + t_b_end[1]);
                    }
                    // Rotate
                    if (item.hasAttribute('data-lax-rotate')) {
                        var t_r = item.getAttribute('data-lax-rotate').split(','),
                            ...
                            t_r_end = t_r[1].split(' ');
                        item.setAttribute('data-lax-rotate', t_r_end[0] + ' ' + t_r_start[1] + ',' + Math.floor(top) + ' ' + t_r_end[1]);
                    }
                    // Scale
                    if (item.hasAttribute('data-lax-scale')) {
                        var t_s = item.getAttribute('data-lax-scale').split(','),
                            ...
                            t_s_end = t_s[1].split(' ');
                        item.setAttribute('data-lax-scale', t_s_end[0] + ' ' + t_s_start[1] + ',' + Math.floor(top) + ' ' + t_s_end[1]);
                    }
                }
            });
