(function ($,Themify, window, topWindow,document, api, Common) {
    ...
    api.Views.register_module({
        ...
        previewReload: function (settings, selector, value) {
            ...
            delete settings['cid'], settings['unsetKey'],settings['element_id'];
    ...
    api.render_element = function (constructData) {
        ...
        // Get ready to load unloaded GS in data
        var keys = Object.keys( api.GS.styles );
        if(undefined !== themifyBuilder.globalStyles && ( Object.keys(themifyBuilder.globalStyles).length <= keys.length )){
            var loaded = [];
            for (var i = keys.length - 1; i > -1; --i) {
                loaded.push( keys[i] );
            }
            data.loadedGS = loaded;
        }
        return $.ajax({
            type: 'POST',
            url: themifyBuilder.ajaxurl,
            ...
            success: function (data) {
                ...
                if(data.gs !== undefined){
                    var keys = Object.keys( data.gs );
                    for (var i = keys.length - 1; i > -1; --i) {
                        api.GS.styles[keys[i]] = data.gs[keys[i]];
                        //set live css style
    ...
    ThemifyLiveStyling = (function ($) {
        ...
        ThemifyLiveStyling.prototype.init = function (isInline) {
            this.type = ThemifyConstructor.component;
            this.group = this.type === 'module'?api.activeModel.get('mod_name'):this.type;
            this.prefix = _.isEmpty(api.activeModel.attributes['gs']) ? ThemifyStyles.getBaseSelector(this.group,api.activeModel.get('element_id')) : '.' + api.GS.styles[api.activeModel.attributes['gs']].class;
        ...
        ThemifyLiveStyling.prototype.setCss = function (data,type,gsClass) {
             var css = ThemifyStyles.createCss(data,type,undefined,gsClass),
                fonts =[];
             // Check and attach used GS in this post
                if(themifyBuilder['globalStyles']!==undefined && gsClass == undefined && Object.keys(themifyBuilder.globalStyles).length>0 ){
                    css = api.GS.attachCSS(css,[],true);
                }
                for(var p in  css){
        ...
        ThemifyLiveStyling.prototype.addOrRemoveFrame = function (_this,location) {
            var self = this,
                $el = this.$liveStyledElmt,
                $context = this.$context.find(self.styleTab),
                side = typeof _this === 'string' ? _this : _this.closest('.tb_tab').id.split('_').pop(),
                settings = {},
            ...
            if ((settings.type === side + '-presets' && settings.layout === '') || (settings.type === side + '-custom' && settings.custom === '')) {
                self.setLiveStyle('backgroundImage', '', selector);
                $frame.remove();
                // Check for GS frames
                ...
                el.addEventListener(event, function (e) {
                    ...
                    if (is_select===true) {
                        if (prop === 'font-weight') {
                            ...
                            if (font!=='' && font !== 'default' && ThemifyConstructor.font_select.safe[font] === undefined) {
    ...
    function verticalResponsiveBars() {
        ...
        api.EdgeDrag = {
            ...
            liveinstance:null,
            init:function(){
                api.Instances.Builder[api.builderIndex].el.addEventListener('mousedown',this.drag.bind(this));
            },
                ...
                var lightbox = el.closest('.tb_dragger_lightbox'),
                    ...
                    for(var i=items.length-1;i>-1;--i){
                        if(items[i].classList.contains('tb_dragger_'+type)){
                            ...
                            this.setData(model,items[i].getAttribute('data-id'),res,u);
                    ...
                    if(api.activeModel!==null && model.cid===api.activeModel.cid){
                        ...
                        if(field!==null){
                            if(field.classList.contains('themify-checkbox')){
                                ...
                                if(field!==null){
                                    field.value=ThemifyConstructor.values[id+'_unit']=u;
                                }
                            }
            ...
            addUndo:function(before,before_settings){
                ...
                var cid = before[0].getAttribute('data-cid'),
                    ...
                    before[0].classList.remove('tb_element_selected');
                    before[0].classList.remove('tb_element_clicked');
                    after[0].classList.remove('tb_element_selected');
                        ...
                        _up = function(e){
                            if(isMoved===true && baseEl!==null){
                                ...
                                for(var i=items.length-1;i>-1;--i){
                                    ...
                                    items[i].setAttribute('data-u',u);
                                    items[i].classList.remove('tb_dragger_dragged');
                                    self.setData(model,items[i].getAttribute('data-id'),current,u);
                            ...
                            else if(e.target===ev.target && e.target.classList.contains('tb_dragger')){
                                api.ActionBar.disable=null;
                            }
