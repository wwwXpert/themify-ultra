    ...
    ThemifyConstructor = {
            ...
            if (data !== false) {//cache visual templates
                ...
                $.ajax({
                    ...
                    success: function (resp) {
                        if (resp) {
                            localStorage.setItem('tb_dragger', JSON.stringify(resp.d));
                            result(resp.v);
                            self.set(resp.v);
        ...
        binding: function (_this, data, val, context) {
            ...
            if (logic) {
                ...
                var getData = function (options, key) {
                    var value,
                ...
                var self = this;
                var relatedBinding = function(el, data,context){
                ...
                for(var i = relatedBindings.length -1 ; i>=0;--i){
        ...
        run: function (type, save_opt) {
            ...
            if (model!== null) {
                ...
                if(api.GS.isEmpty(model.attributes['gs'])){
                    ...
                    createTab = function (index, options) {
                        ...
                        if (index === 'visibility' || index === 'animation') {
                            ...
                            if(globalStylesHTML){
                                ...
                                if(!api.GS.isEmpty(ThemifyConstructor.getStyleVal('global_styles'))){
                                    tab_container.appendChild(api.GS.stylingOverlay());
                                }
                            }
                            fr.appendChild(div);
                            div = null;
            ...
            for (var k in data) {
                ...
                var li = document.createElement('li'),
                        ...
                        label= api.activeModel!==null && !api.GS.isEmpty( api.activeModel.attributes['gs']) ? themifyBuilder.i18n.g_s : data[k].name !== undefined ? data[k].name : this.label[k],
            ...
            if (save_opt !== undefined || model) {
                ...
                if(api.activeModel!==null && !api.GS.isEmpty( api.activeModel.attributes['gs'])){
                    api.GS.modulePanel(true);
                    save.textContent = this.label['s_s'];
                }
                else{
        ...
        saveComponent: function (autoSave) {
            // Check if save GS post
            if(!api.GS.isEmpty(api.activeModel.attributes['gs'])){
                api.GS.saveLiveEdit();
                return true;
            }
            api.saving = true;
            ...
            if (api.mode === 'visual') {
                ...
                topWindow.jQuery('body').triggerHandler('themify_builder_save_component');
            }
        ...
        resetStyling: function (e) {
            ...
            // Reset GS
            var selectedGS = context.getElementsByClassName('tb_selected_style');
            for(var i = selectedGS.length-1;i>-1;--i){
                if ( api.GS.delete( selectedGS[i].getAttribute( 'data-css' ), selectedGS[i].getAttribute( 'data-id' ) ) ) {
                    selectedGS[i].outerHTML = '';
                }
            }
            var GSContainer = context.getElementsByClassName('tb_gs_not_empty');
            if(GSContainer.length){
                GSContainer[0].classList.remove( 'tb_gs_not_empty' );
            }
            ...
            expand: function (item, data, self) {
                item.addEventListener('click', function (e) {
                    ...
                    if (wrap === undefined) {
                        ...
                        self.is_repeat = true;
                        var pid = this.closest('.tb_accordion_fields').id,
                            orig = null,
                ...
                var _this = this,
                    ...
                    for(var i=0,len=values.length;i<len;++i){
                        if(self.is_new!==true || values[i].show===true){
                            f2.appendChild(this.create(self,data,values[i].type,values[i].id,values,true));
                        }
                    }
        ...
        select: {
            ...
            render: function (data, self) {
                ...
                if ( data['optgroup'] ) {
                    var groupF=document.createDocumentFragment(),
                        optgroups = self.getOptions( data );
                    for ( var k in optgroups ) {
                        if(optgroups[k]['label']!==undefined){
                            var o = document.createElement('optgroup');
                                ...
                                groupF.appendChild( o );
                        }
                        else{
                            groupF.appendChild( this.make_options(optgroups[k],v, self ));
                        }
                    }
                    select.appendChild(groupF);
                } else {
        ...
        font_select: {
            ...
            loadGoogleFonts: function ( fontFamilies, iframe ) {
                ...
                for ( var i = fontFamilies.length - 1; i > -1; --i ) {
                    if ( fontFamilies[i] && this.loaded_fonts.indexOf( fontFamilies[i] ) === -1 && (result.google.indexOf( fontFamilies[i] ) === -1 || result.cf.indexOf( fontFamilies[i] ) === -1) ) {
                        ...
                        if ( this.loaded_fonts.indexOf( f ) === -1 ) {
                            var type;
                            if('undefined' !== themifyBuilder && null !== themifyBuilder.google ){
                                type = themifyBuilder.cf[req[0]] !== undefined ? 'cf' : 'google';
                            }else{
                ...
                if ( result.google.length > 0 || result.cf.length > 0 ) {
            ...
            render: function (data, self) {
                ...
                for (var i = groupKeys.length -1; i >= 0; --i) {
                    ...
                    if (v !== undefined) {
                        ...
                        if ('cf' === groupKeys[i] && this.cf[v] !== undefined) {
                            ...
                            opt.textContent = undefined !== this.cf[v] ? this.cf[v].n : v;
                        }
                ...
                wrapper.addEventListener('focusin', function _focusin() {
                    ...
                    if (v !== undefined) {
                        for(var h = 0,len =groups.length;h<len;++h){
                    ...
                    for(var g = extGroups.length-1;g>=0;--g){
                ...
                (function () {
                    ...
                    self.afterRun.push(function () {
                        ...
                        for (var i = 0, len = opt.length; i < len; ++i) {
                            ...
                            else if (opt[i] === 'tax_id' && _data['term_id']===undefined  ){
                                _data['control'] = {control_type: 'queryPosts'};
                            }
                ...
                var callback=function(sl){
                    ...
                    var args = {
                        drag: function (e, ui) {
                            ...
                            if (ui.position.top < 5) {
                                if(ui.position.left < 5) {
                                    ui.position.top = 0;
                                    ui.position.left = 0;
                                } else if(ui.position.left > (boxWidth/2 - 5) && ui.position.left < (boxWidth/2 + 5)) {
                                    ui.position.top = 0;
                                    ui.position.left = (boxWidth/2);
                                } else if(ui.position.left > (boxWidth - 5)) {
                                    ui.position.top = 0;
                                    ui.position.left = boxWidth;
                                }
                                }
                            if ((ui.position.top >= 5 && ui.position.top <= 35) || (ui.position.top >= 45 && ui.position.top <= 75)) {
                                if (ui.position.left < 0) {
                                    ui.position.left = 0;
                                } else if (ui.position.left > boxWidth) {
                                    ui.position.left = boxWidth;
                            }
                            }
                            if (ui.position.top > 35 && ui.position.top < 45) {
                                if(ui.position.left < 5) {
                                    ui.position.top = 40;
                                ui.position.left = 0;
                                } else if(ui.position.left > (boxWidth/2 - 5) && ui.position.left < (boxWidth/2 + 5)) {
                        ...
                        create: function (e, ui) {
                            ...
                            var val = value.split(',');
                            $(handler).css({'left': val[0] + '%', 'top': val[1] + '%'});
                            label.textContent = setPosition(value);
                            value = null;
                        },
                        stop: function (e, ui) {
                            var l = +((ui.position.left/boxWidth)*100).toFixed(2),
                                t = +((ui.position.top/boxHeight)*100).toFixed(2),
                                activeEl=document.querySelector('.tb_element_clicked');
                            input.value=l+','+t;
                            label.textContent = setPosition(l + ',' + t);
                            if (activeEl) {
                                activeEl.setAttribute('style', 'background-position:' + l + '% ' + t + '%;');
                            }
                        }
                    };
                    ...
                    topWindow.$.fn.draggable.call($(sl),args);
                };
                if(this.is_loaded===null){
                    this.loaded = true;
                    topWindow.Themify.LoadAsync(themifyBuilder.includes_url + 'js/jquery/ui/draggable.min.js', function () {
                        callback(this);
                    }.bind(handler), tbLocalScript.version, null, function () {
                        return topWindow.$.fn.draggable !== undefined;
                    });
                }
                else{
                    self.afterRun.push(function(){
        ...
        slider_range: {
            ...
            render: function(data, self) {
                ...
                var callback=function(sl){
                    ...
                    topWindow.$.fn.slider.call($(sl),args);
                };
                if(this.is_loaded===null || topWindow.$.fn.slider===undefined){
                    topWindow.Themify.LoadAsync(themifyBuilder.includes_url + 'js/jquery/ui/slider.min.js', function () {
                        _this.loaded = true;
                        callback(this);
                    }.bind(slider), tbLocalScript.version, null, function () {
                        return topWindow.$.fn.slider !== undefined;
                    });
                }
                else{
                    self.afterRun.push(function(){
        ...
        range: {
            ...
            setData: function (range, item) {
                ...
                range.setAttribute('data-increment',increment);
            },
            controlChange: function (range, unit, event) {
                var is_select = unit !== undefined && unit.tagName === 'SELECT',
                        _this = this;
                function changeValue(condition) {
                    var increment = range.getAttribute('data-increment');
                    if (!increment) {
                        return;
                    }
                    var is_increment = increment % 1 !== 0,
                            max = parseFloat(range.getAttribute('data-max')),
                            min = parseFloat(range.getAttribute('data-min')),
                            cval = range.value,
                            v=0,
            ...
            render: function (data, self) {
                ...
                if (data['units'] === undefined) {
                    ...
                    if (keys.length > 1) {
                        ...
                        if (data.units[unit].increment !== undefined) {
                            input.setAttribute('step', parseFloat(data.units[unit].increment));
                        } else {
                            input.setAttribute('step', '1');
                        }
                        if (v < data.units[unit].min) {
                            input.value = data.units[unit].min;
                        } else if (v > data.units[unit].max) {
