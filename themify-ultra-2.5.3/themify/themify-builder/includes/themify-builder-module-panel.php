 ...
 */
global $post;
$pid=!is_object($post)?false:$post->ID;
$ids = array_keys($this->frontend_builder_ids);
$post_id = !empty($ids) && !in_array($pid,$ids,true) ? $ids[0] : $pid;
if (!$post_id) {
    return;
}
elseif(!$pid){
...
$module_categories = array(
	...
	'product_single'=>__('Product Single','themify')
);
...
<template id="tb_toolbar_template">
	...
	<ul class="tb_toolbar_menu">
		...
		<ul class="tb_toolbar_down">
			...
			<li class="tb_toolbar_<?php echo strtolower( $b ); ?>_switcher">
                ...
                <?php if(!$is_admin && 'desktop' !== strtolower($b) ): ?>
		...
		<a href="#" class="tb_tooltip" tabindex="-1"><i class="ti-panel"></i><span><?php _e('Edit Mode', 'themify'); ?></span></a>
		<ul>
		    <li class="switch-wrapper">
			<div class="tb_switcher">
			    <label>
				<input type="checkbox" class="tb-checkbox toggle_switch">
	...
	<?php Themify_Global_Styles::breadcrumb(); ?>
	<!-- /Global Styles breadcrumb -->
    </div>
