...
class Themify_Builder_Component_Base {
	...
	$options[] = self::get_expand('b', array(
        ...
        // Add Global Style filed to Rows and Columns
        $options[] = array(
            'id' => 'global_styles'
            ,'type' => 'custom_css'
        );
...
;
	...
	//Add custom attributes html5 data to module container div to show parallax options.
	else if (!Themify_Builder::$frontedit_active
             && ( (
                isset( $fields_args['motion_effects'] )
             && (
                    ( isset( $fields_args['motion_effects']['v'] )
                        && isset( $fields_args['motion_effects']['v']['val']['v_dir'] )
                        && $fields_args['motion_effects']['v']['val']['v_dir'] !== ''
                    )
                    || ( isset( $fields_args['motion_effects']['h'] )
                      && isset( $fields_args['motion_effects']['h']['val']['h_dir'] )
                      && $fields_args['motion_effects']['h']['val']['h_dir'] !== ''
                    )
                    || ( isset( $fields_args['motion_effects']['t'] )
                         && isset( $fields_args['motion_effects']['t']['val']['t_dir'] )
                         && $fields_args['motion_effects']['t']['val']['t_dir'] !== ''
                    )
                    || ( isset( $fields_args['motion_effects']['b'] )
                         && isset( $fields_args['motion_effects']['b']['val']['b_dir'] )
                         && $fields_args['motion_effects']['b']['val']['b_dir'] !== ''
                    )
                    || ( isset( $fields_args['motion_effects']['r'] )
                         && isset( $fields_args['motion_effects']['r']['val']['r_dir'] )
                         && $fields_args['motion_effects']['r']['val']['r_dir'] !== ''
                    )
                    || ( isset( $fields_args['motion_effects']['s'] )
                         && isset( $fields_args['motion_effects']['s']['val']['s_dir'] )
                         && $fields_args['motion_effects']['s']['val']['s_dir'] !== ''
                    )
                )
              )
              || !empty( $fields_args['custom_parallax_scroll_speed'] )
              )
             && Themify_Builder_Model::is_parallax_active()) {
		$props['data-lax'] = 'true';
        ...
        // Vertical
		if ( isset( $fields_args['motion_effects']['v'] )
            && isset( $fields_args['motion_effects']['v']['val']['v_dir'] )
            && $fields_args['motion_effects']['v']['val']['v_dir'] !== '' ) {
		    $v_speed = 600;
		...
		// Horizontal
		if ( isset( $fields_args['motion_effects']['h'] )
		            && isset( $fields_args['motion_effects']['h']['val']['h_dir'] )
		            && $fields_args['motion_effects']['h']['val']['h_dir'] !== '' ) {
		    $h_speed = 600;
		...
		// Blur
		if ( isset( $fields_args['motion_effects']['b'] )
		     && isset( $fields_args['motion_effects']['b']['val']['b_dir'] )
		     && $fields_args['motion_effects']['b']['val']['b_dir'] !== '' ) {
		    $b_level = 10;
		...
		// Rotate
		if ( isset( $fields_args['motion_effects']['r'] )
		     && isset( $fields_args['motion_effects']['r']['val']['r_dir'] )
		     && $fields_args['motion_effects']['r']['val']['r_dir'] !== '' ) {
        ...
        // Scale
		if ( isset( $fields_args['motion_effects']['s'] )
		     && isset( $fields_args['motion_effects']['s']['val']['s_dir'] )
		     && $fields_args['motion_effects']['s']['val']['s_dir'] !== '' ) {
    ...
    private static function map_animation_speed( $val, $attr = 'no' ) {
	    switch ( $val ) {
		    case 10:
                $speed = ($attr === 'blur') ? 20 : 1300;
			    break;

		    case 9:
                $speed = ($attr === 'blur') ? 18 : 1200;
			    break;

		    case 8:
                $speed = ($attr === 'blur') ? 16 : 1100;
			    break;

		    case 7:
                $speed = ($attr === 'blur') ? 14 : 1000;
			    break;

		    case 6:
                $speed = ($attr === 'blur') ? 12 : 800;
			    break;

		    case 4:
                $speed = ($attr === 'blur') ? 8 : 400;
			    break;

		    case 3:
                $speed = ($attr === 'blur') ? 6 : 300;
			    break;

		    case 2:
                $speed = ($attr === 'blur') ? 4 : 200;
			    break;
     ...
     */
	private static function do_color_overlay( $styling, $gs = false ) {
		...
		$result = !($is_empty === true && $is_empty_hover === true );
		if($result===false && !$gs &&  !empty($styling['global_styles']) && !empty(self::$used_gs)){
		    // Try to check attached GS
		    $global_styles = explode(' ',$styling['global_styles']);
		    foreach ($global_styles as $class){
			if(isset(self::$used_gs[$class]) && self::do_color_overlay(self::$used_gs[$class],true)){
			    $result = true;
			    break;
			}
		    }
	...
	protected static function show_frame( $styles, $gs = false ) {
		...
		if( $gs === false && $count < 4 && !empty(self::$used_gs) && !empty($styles['global_styles'])){
		    // Try to check attached GS
		    $global_styles = explode(' ',$styles['global_styles']);
		    foreach ($global_styles as $class){
			if(isset(self::$used_gs[$class])){
			    $printed = self::show_frame(self::$used_gs[$class],$printed);
				if($count === 4){
				    break;
			    }
			}
