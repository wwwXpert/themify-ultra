...
class Themify_Builder_Stylesheet {
    ...
    public static function enqueue_stylesheet( $return = false, $post_id = null ) {
		if((isset(self::$isLoaded[$post_id]) && !Themify_Builder_Model::is_front_builder_activate()) || (defined( 'REST_REQUEST' ) && REST_REQUEST )){
			return false;
		}
				...
				if( $return ) {
					...
					if ( !empty( $fonts ) ) {
						...
						if ( $href !== false ) {
							...
							wp_enqueue_style( $handlerId.'-fonts', $href, null,null);
						    }
						}
					}
					// Custom Fonts
					$fonts = Themify_Custom_Fonts::get_fonts( $stylesheet_path['id'] );
					if ( !empty( $fonts ) ) {
						$href = Themify_Custom_Fonts::$api_url. $fonts ;
						$fonts = null;
						if ( $href !== false ) {
						    if($isLoop===true){
							$link .= '<link class="themify-builder-generated-css" id="' . $handlerId . '-cf-fonts" rel="stylesheet" href="' . $href . '" type="text/css" />';
						    }
						    else{
							$link.='<link rel="preload" href="'.$href.'" as="style"/>';
							wp_enqueue_style( $handlerId.'-cf-fonts', $href, null,null);
						    }
						}
					}
					echo $link;
