	...
	class Themify_Builder_Options {
		...
		public function add_plugin_page(){
			if( Themify_Builder_Model::hasAccess() ){
				...
				if (Themify_Builder_Model::builder_check()) {
					...
					add_submenu_page ( 'themify-builder', __( 'Saved Layouts', 'themify' ), __( 'Saved Layouts', 'themify' ), 'edit_posts', 'edit.php?post_type=tbuilder_layout' );
					add_submenu_page( 'themify-builder', __( 'Layout Parts', 'themify' ), __( 'Layout Parts', 'themify' ), 'edit_posts', 'edit.php?post_type=tbuilder_layout_part' );
					...
					add_submenu_page( 'themify-builder', __( 'Custom Fonts', 'themify' ), __( 'Custom Fonts', 'themify' ), 'edit_posts', 'edit.php?post_type=tb_cf' );
		...
		function before_save($input) {
			...
			if(class_exists('TFCache')){
				$remove = (!empty($all['builder_is_active']) && $all['builder_is_active']!=='enable') || isset($all['builder_disable_cache']);
		...
		function find_and_replace_field(){
			...
			$output .='<br/><br/>';
			$output .= '<input id="builder-find-and-replace-btn-plugin" type="button" name="builder-find-and-replace-btn-plugin" '.$disabled.' value="'.$value.'" class="button big-button"/>';
			$output .='<br/><br/><p class="description">';
