...
if( !class_exists( 'Themify_Builder_Layouts' ) ) {
	...
	class Themify_Builder_Layouts
	{
		...
		public static function register_layout_post_type() {
		...
		public static function register_layout_part_post_type() {
			return new CPT( array(
		...
		public function register_layout() {
			...
			$this->layout = self::register_layout_post_type();
			...
			// create a template custom post type
			$this->layout_part = self::register_layout_part_post_type();
		...
		public function set_layout_ajaxify() {
			...
			if ( !is_wp_error( $builder_data ) && !empty( $builder_data ) ) {
				...
				if ( $layoutPost = get_page_by_path( $_POST['layout_slug'], OBJECT, 'tbuilder_layout' ) ) {
		...
		public function row_actions( $actions ) {
			...
			if ( $this->layout->post_type_name === $post_type || $this->layout_part->post_type_name === $post_type ) {
