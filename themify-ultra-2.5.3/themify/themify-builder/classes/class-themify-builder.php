...
if (!class_exists('Themify_Builder')) :
	...
	public function async_footer() {
		...
		'isGlobalStylePost' => 'tglobal_style' === get_post_type(get_the_ID())
	    ));
	...
	private function load_frontend_interface() {
		...
		switch ($script) {
			...
			wp_enqueue_script($script, themify_https_esc('http://ajax.googleapis.com/ajax/libs/webfont/1/webfont.js'), null, 1, true);
			break;
			...
			wp_localize_script($script, 'themifyBuilder', apply_filters('themify_builder_ajax_front_vars', array(
			    ...
			    'custom_css'=>isset($_GET['id'])?get_post_meta( $_GET['id'], 'tbp_custom_css', true ):'',
	...
	public static function get_i18n() {
	    ...
	    if (!empty($image_size)) {
		$options['options']['image_size'] = $image_size;
	    }
	    return $options;
	}
	...
	private function load_admin_interface() {
		...
		switch ($script) {
			...
			wp_localize_script($script, 'themifyBuilder', apply_filters('themify_builder_ajax_admin_vars', array(
			    ...
			    'debug' => defined('THEMIFY_DEBUG') && THEMIFY_DEBUG,
			    'import_nonce' => wp_create_nonce('themify_builder_import_filethemify-builder-plupload'),
			    'custom_css'=>(isset($_GET['id']) || isset($_GET['post']))?get_post_meta( $id, 'tbp_custom_css', true ):'',
			    // Breakpoints
			    'breakpoints' => themify_get_breakpoints(),
				'cf_api_url' => Themify_Custom_Fonts::$api_url
			)));
	...
	public function builder_stylesheet_style_tag($tag, $handle, $href, $media) {
		if ( 'builder-styles' === $handle ) {
			$tag  = '';
			/* on singular page, prefetch builder stylesheet only if there's builder content */
	...
	public function load_module_partial_ajaxify() {
		...
		Themify_Global_Styles::set_used_gs();
	    }
	    $new_modules = array(
			...
			foreach ( $batch as $b ) {
				...
				switch ( $type ) {
					case 'module':
						...
						$markup = Themify_Builder_Component_Module::template( $b['data'], $b['jobID'], false, array( $b['jobID'] ) );
						break;
					...
					case 'column':
						...
						$markup = Themify_Builder_Component_Column::template( $b['jobID'], array( 'row_order' => $b['jobID'] ), $b['jobID'], $b['data'], $b['jobID'] );
						break;
	...
	public function save_builder_css() {
	    ...
	    if (!empty($data) && is_array($data)) {
			echo json_encode(Themify_Builder_Stylesheet::write_stylesheet((int) $_POST['id'], $data, $_POST['custom_css']));
	    }
	    die;
	}
	...
	function get_builder_output( $post_id, $content = '' ) {
	    ...
	    if (Themify_Builder_Model::is_front_builder_activate() && isset( $_GET['id'] ) && $post_id == $_GET['id'] ) {
			...
			if(!(isset($_GET['page']) && 'themify-global-styles' === $_GET['page']  &&  Themify_Builder_Model::	is_front_builder_activate() && is_admin())){
				Themify_Global_Styles::set_used_gs();
			}
			$template = $this->in_the_loop ? 'builder-layout-part-output.php' : 'builder-output.php';
	...
	public function add_minify_vars($vars) {
	    ...
	    $vars['minify']['js']['themify.parallaxit'] = themify_enque(THEMIFY_BUILDER_URI . '/js/themify.parallaxit.js', true);
	...
	public function load_current_inner_page_content($builder_data, $page_breaks) {
	    ...
	    $multipage = 1;
	    return isset($temp_data[$page]) ? $temp_data[$page] : $builder_data;
	}
	...
	public function themify_regenerate_css_files_ajax() {
		...
		if (!$wp_filesystem->execute->rmdir($path, true)) {
		    die(__('Something goes wrong. Please check the if the upload folder is writtable.'));
		}
	    }
	    die('finished');
	}
    ...
    public function display_custom_css() {
        $custom_css = get_post_meta( get_the_ID(), 'tbp_custom_css', true );
        if ( $custom_css ){
	...
	function find_text_inputs_in_module($options) {
	    $inputs = array();
	    $allowed_types = array('text', 'textarea', 'wp_editor', 'custom_css', 'image');
		...
		public function themify_find_and_replace_ajax() {
			...
			foreach ( $posts_id as $post_id ) {
				...
				if ( $data !== $new_data ) {
					...
					$data = json_decode( stripslashes_deep( $new_data ), true );
					$data = null === $data ? json_decode( $new_data, true ) : $data;
					if ( is_array( $data ) ) {
	...
	public function tb_get_autocomplete() {
		if ( ! isset( $_POST['dataset'], $_POST['value'], $_POST['dataset'] ) || empty( $_POST['dataset'] ) ) {
			wp_send_json_error();
		}
		$pid = (int) $_POST['pid'];
	...
	function search_custom_fields( $name ) {
		...
		$results = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT DISTINCT meta_key FROM {$wpdb->prefix}postmeta WHERE meta_key like %s LIMIT 100",
				'%' . $name . '%'
			),
		OBJECT );
