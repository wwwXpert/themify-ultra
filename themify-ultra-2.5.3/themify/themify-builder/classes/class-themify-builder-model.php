...
final class Themify_Builder_Model {
	...
	static $is = NULL;
	if ($is === null) {
	    $is = is_user_logged_in() && current_user_can('edit_pages', get_the_ID());
	...
	if ($is === null) {
	    ...
	    $is = self::is_front_builder_activate() || !( ( $disable_all || ( $disable_mobile && themify_is_touch() ) ) );
	}
	return $is;
    }
	...
	if ($is === null) {
	    ...
	    $disable_all = $val === 'all';
	    $disable_mobile = $disable_all || $val === 'mobile';
	    $is = !( ( $disable_mobile && themify_is_touch() ) || $disable_all );
	}
	return $is;
    }
	...
	$is = null;
	if ($is === null) {
	    $is = !( defined('THEMIFY_BUILDER_VERSION') && preg_match('/[1-9].[0-9].[0-9]/', THEMIFY_BUILDER_VERSION) );
	}
	return $is;
    }
	...
	return array(array(
		...
		'type' => 'checkbox',
		'options' => array(
		    array('name' => 'vertical', 'value' => __('Display arrows vertical middle', 'themify'))
		)
	    ),
	    array(
