	...
	<div class="module-<?php echo $args['mod_name']?>">
		...
		$icon =$content['icon']? sprintf( '<i class="%s"></i>', themify_get_icon($content['icon'] )):'';
		    ...
		    <?php if( $content['icon_alignment'] !== 'right' && $icon!==''):?>
			    <?php echo $icon?>
		    <?php endif;?>
			<span<?php if(Themify_Builder::$frontedit_active):?> contenteditable="false" data-name="label" data-repeat="content_button" data-index="'.$i.'"<?php endif;?>><?php echo $content['label']?></span>
		    <?php if( $content['icon_alignment'] === 'right' && $icon!==''):?>
			    <?php echo $icon?>
		    <?php endif;?>
		    <?php if( $content['link'] ):?>
