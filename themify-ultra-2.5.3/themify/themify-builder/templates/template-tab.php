...
if (TFCache::start_cache($args['mod_name'], self::$post_id, array('ID' => $args['module_ID']))):
	...
	$tab_id = $args['element_id'];
	$container_class[] = 'tb_'.$tab_id;
    }
    else{
	$tab_id = $args['module_ID'] . '-' . $args['builder_id'];
    }
        ...
        <div class="builder-tabs-wrap">
                ...
                <?php foreach ($fields_args['tab_content_tab'] as $k => $tab): ?>
                    <li <?php echo 0 === $k ? 'aria-expanded="true"' : 'aria-expanded="false"'; ?>>
                        <a href="#tab-<?php echo $tab_id . '-' . $k; ?>">
                            ...
                            <span<?php if(Themify_Builder::$frontedit_active):?> contenteditable="false" data-name="title_tab" data-index="<?php echo $k?>" data-repeat="tab_content_tab" <?php endif; ?>><?php echo isset($tab['title_tab']) ? $tab['title_tab'] : ''; ?></span>
                        </a>
                    </li>
                <?php endforeach; ?>
            </ul>
            ...
            <?php foreach ($fields_args['tab_content_tab'] as $k => $tab): ?>
                <div id="tab-<?php echo $tab_id . '-' . $k; ?>" class="tab-content" <?php echo $k === 0 ? 'aria-hidden="false"' : 'aria-hidden="true"' ?>>
                    <div class="tb_text_wrap<?php if(Themify_Builder::$frontedit_active):?> tb_editor_enable<?php endif; ?>"<?php if(Themify_Builder::$frontedit_active):?> contenteditable="false" data-name="title_tab" data-index="<?php echo $k?>" data-repeat="tab_content_tab"<?php endif; ?>>
		    <?php
