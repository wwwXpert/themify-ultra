...
class Themify_Metabox {
	...
	function get_meta_boxes() {
		...
		if( ! isset( $meta_boxes ) ) {
			...
			// Don't show on Custom Font Type
			unset($types[Themify_Custom_Fonts::$customFont->post_type_name]);
	...
	function admin_enqueue_scripts( $page = '' ) {
		...
		wp_register_style( 'themify-metabox', themify_enque(THEMIFY_METABOX_URI . 'css/styles.css'), array( 'themify-colorpicker', 'themify-datetimepicker' ) );
		...
		wp_register_script( 'themify-metabox', themify_enque(THEMIFY_METABOX_URI . 'js/scripts.js'), $themify_metabox_scripts, '1.0', true );
		wp_register_script( 'themify-plupload',themify_enque(THEMIFY_METABOX_URI . 'js/plupload.js'), array( 'jquery', 'themify-metabox' ), null, true );
		...
		if ( isset( $_FILES['file'] ) ) {
			...
			if ( $new_data !== null ) {
				...
				if ( ($end===true && 'colors' === $type) || ( $end===false && 'gradients' === $type ) ) {
					...
					delete_option($_key);
					add_option($_key,serialize( $new_data ), '', false );
	...
	function themify_save_colors() {
		check_ajax_referer( 'ajax-nonce', 'tb_load_nonce' );
		$type = $_POST['type'];
		$colors = !empty( $_POST['colors'] ) ? serialize( $_POST['colors'] ) : serialize( array() );
		$_key='themify_saved_' . $type;
		delete_option($_key);
	...
	public static function themify_localize_cm_data() {
		...
		$data = array(
			...
			'gradients' => unserialize( get_option( 'themify_saved_gradients', serialize( array() ) ) ),
