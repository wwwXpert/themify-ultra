...
function themify_theme_enqueue_scripts() {
	...
	wp_enqueue_style( 'theme-style', themify_enque( THEME_URI . '/style.css' ), null, $theme_version);

	// Themify Media Queries CSS
	wp_enqueue_style( 'themify-media-queries', themify_enque(THEME_URI . '/media-queries.css'), null, $theme_version);
	...
	if (  $show_menu_navigation ) {
		...
		if ( ! empty( $menu_style ) ) {
			wp_enqueue_style( 'themify-mobile-menu-' . $menu_style , themify_enque( THEME_URI . '/styles/' . 'menu-'. $menu_style . '.css' ), null, $theme_version );
		}
    }
	...
	if( is_child_theme() ) {
	   if(is_rtl())
		wp_enqueue_style( 'theme-style-rtl', themify_enque( THEME_URI . '/rtl.css' ), null, $theme_version);
	
		wp_enqueue_style( 'theme-style-child', themify_enque( get_stylesheet_uri() ), null, $theme_version );
	}
	...
	if ( $is_fullpage_scroll ){
	    wp_enqueue_style( 'themify-section-scroll', themify_enque(THEME_URI . '/styles/section-scroll.css'), null, $theme_version);
	    if ( (class_exists('Themify_Builder_Model') && !Themify_Builder_Model::is_front_builder_activate())) {
		wp_enqueue_style( 'themify-splitscroll', themify_enque(THEME_URI . '/styles/splitscroll.css'), null, $theme_version);
	...
	if ('slider' !== $themify->post_layout && ( is_home() || is_archive() || themify_is_query_page() || is_search() )  ) {
		// Infinite scroll
